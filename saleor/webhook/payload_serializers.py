...
class PayloadSerializer(JSONSerializer):
    def __init__(
        self, extra_model_fields: dict[str, Sequence[str]] | None = None
    ) -> None:
        super().__init__()
        self.extra_model_fields = extra_model_fields or {}
        # Any: caller-provided `(getter, fields)` pairs and extra values are
        # arbitrary and are only introspected at runtime in `get_dump_object`.
        self.additional_fields: dict[str, tuple[Any, Any]] = {}
        self.extra_dict_data: dict[str, Any] = {}
        self.obj_id_name = "id"
        self.pk_field_name = "id"
        self.dump_type_name = True

    # `options` are forwarded verbatim to Django's serializer, which accepts
    # arbitrary keyword options.
    def serialize(self, queryset: Iterable[Model], **options: Any) -> str:
