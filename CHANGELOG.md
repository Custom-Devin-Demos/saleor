...
- The `saleor.checkout`, `saleor.discount`, `saleor.order`, `saleor.payment` and `saleor.warehouse` packages now pass `mypy --strict`, enforced in CI by the new `Mypy strict` workflow (`scripts/mypy_strict.sh <package>` locally). `celery-types` was added as a dev dependency to type Celery tasks.
